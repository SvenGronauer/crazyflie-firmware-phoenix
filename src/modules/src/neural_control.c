...
void safetyController(droneState_t *droneState, controlState_t *controlState)
{
  if (droneState->x < -2.5 || droneState->x > 2.5 ||
      ...
      droneState->roll_dot < (-400 * deg2rad) || droneState->roll_dot > (400 * deg2rad) ||
      droneState->pitch_dot < (-400 * deg2rad) || droneState->pitch_dot > (400 * deg2rad))
  {
...
// TODO: Need sensor data
{
  ...
  case CONTROL_MODE_ATTITUDE_RATE:
    ...
    *commandSetpoint = attRateSetpoint;
    ...
    /* =========================
    *  PID Attitude Rate Updates
    */
    // int16_t thrust = neuralSetpoint->clippedValue[0]  // in [0, 65535]
    // int16_t desired_roll_rate = neuralSetpoint->clippedValue[1]; // in [deg/s]
    // int16_t desired_pitch_rate = neuralSetpoint->clippedValue[2];  // in [deg/s]
    // int16_t desired_yaw_rate = neuralSetpoint->clippedValue[3];  // in [deg/s]


    // //TODO: Need sensor data
    // updateNeuralAttitudeRateController(
    //   sensors->gyro.x, sensors->gyro.y, sensors->gyro.z,
    //   desired_roll_rate, desired_pitch_rate, desired_yaw_rate);

    // // fetch current values from Neural Atttitude Rate Controller
    // getActuatorOutputNeuralAttitudeRate(neuralRollRate, neuralPitchRate, neuralYawRate);

    // int16_t thrust = neuralSetpoint->clippedValue[0]
    // int16_t r = neuralRollRate / 2.0f;
    // int16_t p = neuralPitchRate / 2.0f;
    // // int16_t yaw = neuralYawRate;
    ...
    // motorsSetRatio(0, limitThrust(thrust - r + p + neuralYawRate);
    // motorsSetRatio(1, limitThrust(thrust - r - p - neuralYawRate));
    // motorsSetRatio(2, limitThrust(thrust + r - p + neuralYawRate);
    // motorsSetRatio(3, limitThrust(thrust + r + p - neuralYawRate);
    break;
  case CONTROL_MODE_ATTITUDE:
    attSetpoint.attitude.pitch = neuralSetpoint->value[0];
    attSetpoint.attitude.roll = neuralSetpoint->value[1];
    attSetpoint.attitude.yaw = neuralSetpoint->value[2];
    attSetpoint.thrust = neuralSetpoint->value[3];
